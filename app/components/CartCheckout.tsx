...
export default function CartCheckout({}: Props) {
	const { cartOpen, setCartOpen, cart, updateCartItem } = useContext(StoreContext)
	...
	async function handleCheckoutProductsCart() {
		...
		setIsLoading(true)
		const lineItems = cart.map(prod => ({ variantId: prod.variants?.edges[0].node.id, quantity: prod.quantity }))
	...
	return (
		<div className={`fixed w-screen h-screen z-10 top-0 left-0 flex ${cartOpen ? 'visible' : 'invisible'}`}>
			<div onClick={() => setCartOpen(!cartOpen)}
					 className={`absolute w-full h-full top-0 left-0 ${cartOpen ? 'backdrop-blur-sm' : 'backdrop-blur-none'}`} />
			<div
				className={`absolute flex flex-col h-full justify-between w-11/12 sm:w-5/12 h-full top-0 right-0 bg-white shadow-sm py-12 px-8 transition-all duration-300 ${cartOpen ? 'right-0' : '-right-[100%]'}`}>
									...
									>
										...
										<div className='flex justify-between w-full items-center'>
											<div className='mt-4 text-base font-bold text-gray-900 h-full'>
												...
												<p>
													{formatPrice(
														product.priceRange.minVariantPrice.amount * (product.quantity || 1),
													)}
												</p>
											...
											<div className='flex flex-col items-center gap-y-2'>
												<div
													className='bg-gray-50 rounded-md py-3 px-2 flex flex-col items-center justify-between text-base text-gray-500 gap-2'>
													<button
														className='hover:text-gray-900'
														onClick={() => updateCartItem(variantId as string, {
															...product,
													...
													<input
														...
														onBlur={() => {
															if (!product.quantity) updateCartItem(variantId as string, { ...product, quantity: 1 })
														}}
														onChange={(el) =>
													...
													<button
														...
														onClick={() =>
															updateCartItem(variantId as string, {
																...product,
																quantity: Math.max((product.quantity as number) - 1, 1),
															})
														}
													>
												...
												<button onClick={() => updateCartItem(variantId as string, {} as SingleProduct)}
				...
				<div className='mt-10 grid grid-cols-1 gap-x-6 gap-y-4 sm:grid-cols-8'>
					<div
						className='w-full bg-white rounded-md py-3 px-8 flex items-center justify-center text-lg font-medium text-gray-900 col-span-4'>
						<p>
							<span className='text-gray-500'>Total{' '}</span>
							{formatPrice(cart.reduce((val, prod) => (prod.priceRange.minVariantPrice.amount * (prod.quantity as number)) + val, 0))}
						</p>
					</div>
					<button
