					...
					<div className='h-16 flex items-center justify-between'>
						<div className='flex-1 flex'>
							<Link to='/'>
								<a className='flex items-center'>
									...
									<span className='ml-2 uppercase text-2xl font-bold tracking-tight'>Muggs Shop</span>
								</a>
							</Link>
						</div>
						<div className='flex-1 flex items-center justify-end'>
							...
							<button onClick={() => setCartOpen(!cartOpen)}
								...
								<span
									className='text-xs text-white absolute top-[2px] right-[2px] bg-gray-600 w-4 h-4 tracking-tight leading-4 font-thin rounded-full'>{cart.reduce((val, prod) => (prod.quantity as number || 0) + val, 0)}</span>
